...
Node* create(unsigned char arr[4][4]){
    ...
    for (int i=0 ;i<4;i++){
        ...
        if (newNode==NULL){Node* current = head;
            while (current != NULL) {
                Node* temp = current;
...
node* createList( char** list, int node_number) {
    ...
    for (int k = 0; k < 16; k++) {
        int row = k / 4;
        int col = k % 4;
        n->array[row][col] = list[node_number][k];
    }
     //printf("list created\n");
    return n; 
}
...
void subByte(node* state)
{
    ...
    //printf("bytes subbed /n");
}
void printres(node* res){
     node* current=res;
    int count=0;
    while (current!=NULL){
        for(int i=0;i<4;i++){
            for(int j=0;j<4;j++){
                 printf("%02X ",current->array[i][j]);
            }
...
Node* expansion_key( unsigned char epanK[4][4])
{
    Node* head = create(epanK);
...
int main() {
    ...
    Node* exp = expansion_key(expanK);
    node* hihi= NULL;
    node* tail = NULL;
    for (int i = 0; i < l; i++) {
        node* block = createList(r, i);  
        if (hihi == NULL) {
            hihi = block;
            tail = block;
        } else {
