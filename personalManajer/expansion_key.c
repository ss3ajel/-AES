...
int main(){
    ...
    while(round < 10){
        ...
        for(int w = 0; w < 4; w++){
            Node* bababoy = (Node*)malloc(sizeof(Node));
            for(int i = 0; i < 4; i++){
                bababoy->data[i] = tail->data[i];
            }
            bababoy->next = NULL;

            if(w == 0){
                permutation(bababoy);
                sub(SBox, bababoy, RCi, round);
            }

            unsigned char newdata[4];
            for(int i = 0; i < 4; i++){
                newdata[i] = bababoy->data[i] ^ prev4->data[i];
            }
            free(bababoy);

            tail = append(tail, newdata);
            prev4 = prev4->next;
        }
        round++;
    }
